...
> `AGENTS.md` espelha este arquivo (versão para o Codex e outros agentes) — toda alteração aqui deve ser replicada lá.
...
- Use a skill de projeto **`rebobine-docs`** (`.claude/skills/rebobine-docs/`): **antes** de alterar (consultar o capítulo certo, regras afetadas, armadilhas conhecidas) e **depois de toda entrega**.
...
- **Uma entrega sem a DOC atualizada não está pronta.** Checklist de fecho: skill **`verificar-entrega`** (`.claude/skills/verificar-entrega/`).
